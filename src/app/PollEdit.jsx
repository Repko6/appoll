...
export const PollEdit = ({ match }) => {
  ...
  async function addAnswer(pollQuestionId) {
    const newSort = pollQuestions.find(c => c.id == pollQuestionId).answers.sort(function (a, b) { return a.sort - b.sort })[pollQuestions.find(c => c.id == pollQuestionId).answers.length - 1].sort + 1;
    const newId = pollQuestions.find(c => c.id == pollQuestionId).answers.sort(function (a, b) { return a.id - b.id })[pollQuestions.find(c => c.id == pollQuestionId).answers.length - 1].id + 1;
    let hasOtherAnswer =pollQuestions.find(c => c.id == pollQuestionId).answers.some(d => d.name === 'Drugo');

    let data = pollQuestions.map(item =>
      item.id == pollQuestionId
        ? { ...item, answers: [...item.answers.map(item2 => item2.name === 'Drugo' ? { ...item2, id: newId, sort: newSort } : item2), { id: hasOtherAnswer ? newId - 1 : newId,sort: hasOtherAnswer ? newSort - 1 : newSort, name: `Odgovor ${newId}` }] }
        : item);
                            ...
                            <div>
                              {c.answers.sort(function (a, b) { return a.sort - b.sort }).map(d =>
                                <div key={`answer_${d.id}`}>
                                  ...
                                  <button onClick={() => removeAnswer(c.id, d.id)} >❌</button>
